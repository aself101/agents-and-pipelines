---
name: operators-eye
description: Models the perspective of an SRE or platform engineer running the artifact in production. Surfaces failure-mode visibility, recovery primitives, blast radius, and time-to-diagnose from the 3am pager perspective. Decision OPERABLE/OPAQUE_IN_PROD.
---

# Operator's Eye v1
Models the perspective of an SRE or platform engineer running the artifact in production. Surfaces failure-mode visibility, recovery primitives, blast radius, and time-to-diagnose from the 3am pager perspective. Decision OPERABLE/OPAQUE_IN_PROD.
...
**Usage:** `/agents:operators-eye <directory>`

**Examples:**
- `/agents:operators-eye src/server.ts`
- `/agents:operators-eye docker-compose.yml`
- `/agents:operators-eye infrastructure/`
- `/agents:operators-eye src/services/`
...
```bash
echo "Running operator's eye analysis on $ARGUMENTS..."
echo "================================================"
```
...
Run the Operator's Eye agent on the validated target directory:

**Agent:** operators-eye-agent.md
**Model:** Opus
**Target:** $ARGUMENTS
...
| ID | Condition |
|----|-----------|
| **AF-001** | Generic reliability checklist presented as operator perspective analysis |
| **AF-002** | Builder perspective presented as operator perspective |
| **AF-003** | Findings not grounded in operator perspective |
...
Artifact OPERABLE — operator can diagnose, contain, and recover from failures
...
Artifact OPAQUE_IN_PROD — operator lacks visibility, recovery primitives, or blast-radius understanding
...
| `definition_name` | `operators-eye` | From CDL interface |
...
**CDL Source:** `/Users/aself/uluops/uluops-agent-workflows/udl/cdl/v1/operators-eye.command.yaml`
**Agent:** `agents/operators-eye-agent.md`
