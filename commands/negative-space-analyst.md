...
description: Reads meaning from what an artifact deliberately does not include. Identifies principled silences (design commitments) and revealing silences (unconscious assumptions exposed by omission). Decision INTENTIONAL/REVEALING.
---

# Negative Space Analyst v1
Reads meaning from what an artifact deliberately does not include. Identifies principled silences (design commitments) and revealing silences (unconscious assumptions exposed by omission). Decision INTENTIONAL/REVEALING.
...
**Usage:** `/agents:negative-space-analyst <directory>`

**Examples:**
- `/agents:negative-space-analyst udl/adl/v3/code-validator.agent.yaml`
- `/agents:negative-space-analyst docs/architecture.md`
- `/agents:negative-space-analyst specs/api-spec.md`
- `/agents:negative-space-analyst src/`
...
```bash
echo "Running negative space analysis on $ARGUMENTS..."
echo "================================================"
```
...
Run the Negative Space Analyst agent on the validated target directory:

**Agent:** negative-space-analyst-agent.md
**Model:** Opus
**Target:** $ARGUMENTS
...
| ID | Condition |
|----|-----------|
| **AF-001** | Feature request list presented as negative space analysis |
| **AF-002** | No principled/revealing classification — all silences treated equivalently |
| **AF-003** | No scope calibration — absences identified without establishing what is in scope |
...
Artifact INTENTIONAL — silences are principled design commitments
...
Artifact REVEALING — omission patterns expose unconscious assumptions
...
| `definition_name` | `negative-space-analyst` | From CDL interface |
...
**CDL Source:** `/Users/aself/uluops/uluops-agent-workflows/udl/cdl/v1/negative-space-analyst.command.yaml`
**Agent:** `agents/negative-space-analyst-agent.md`
