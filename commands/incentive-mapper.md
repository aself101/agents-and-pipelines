...
description: Identifies what behaviors the artifact creates — intended or not. Every scoring framework, policy, workflow, and agent definition creates incentive gradients. Makes the gradient visible before it's exploited. Produces a ranked incentive inventory with optimization targets, penalty avoidance patterns, and gaming scenarios.
---

# Incentive Mapper v1
Identifies what behaviors the artifact creates — intended or not. Every scoring framework, policy, workflow, and agent definition creates incentive gradients. Makes the gradient visible before it's exploited. Produces a ranked incentive inventory with optimization targets, penalty avoidance patterns, and gaming scenarios.
...
**Usage:** `/agents:incentive-mapper <directory>`

**Examples:**
- `/agents:incentive-mapper udl/adl/v3/code-validator.agent.yaml`
- `/agents:incentive-mapper agents/v3/assumption-excavator-agent.md`
- `/agents:incentive-mapper docs/specs/meta-layer-agents-spec.md`
- `/agents:incentive-mapper specs/api-spec.md`
...
```bash
echo "Running incentive mapping on $ARGUMENTS..."
echo "=========================================="
```
...
Run the Incentive Mapper agent on the validated target directory:

**Agent:** incentive-mapper-agent.md
**Model:** Opus
**Target:** $ARGUMENTS
...
| ID | Condition |
|----|-----------|
| **AF-001** | No unintended incentives found in a scoring or evaluation artifact |
| **AF-002** | Incentives described without the actor who responds to them |
| **AF-003** | Only intended incentives documented — intended does not equal actual |
| **AF-004** | No gaming scenario — how would a rational actor exploit this? |
...
Artifact ALIGNED — incentive gradients produce intended behaviors
...
Artifact MISALIGNED — incentive gradients reward unintended behaviors, realign before deployment
...
| `definition_name` | `incentive-mapper` | From CDL interface |
...
**CDL Source:** `/Users/aself/uluops/uluops-agent-workflows/udl/cdl/v1/incentive-mapper.command.yaml`
**Agent:** `agents/incentive-mapper-agent.md`
