---
name: state-validate
description: Validate stateful workflows and multi-step interactions. Tests authentication flows, shopping carts, pagination, session management, and state transitions through actual request sequences.
---
...
**Usage:** `/agents:state-validate <directory>`

**Examples:**
- `/agents:state-validate ./api`
- `/agents:state-validate ./services/api`
- `/agents:state-validate .`
...
```bash
echo "Running state validation on $ARGUMENTS..."
echo "========================================="
```
...
Run the State Validator agent on the validated target directory:

**Agent:** state-validator-agent.md
**Model:** Sonnet
**Target:** $ARGUMENTS
...
State validation passed - workflows are CONSISTENT (score >= 75)
...
State validation failed - workflows are INCONSISTENT. Review state issues above.
...
| `definition_name` | `state-validate` | From CDL interface |
...
**CDL Source:** `/Users/aself/uluops/uluops-agent-workflows/udl/cdl/v1/state-validate.command.yaml`
**Agent:** `agents/state-validator-agent.md`
