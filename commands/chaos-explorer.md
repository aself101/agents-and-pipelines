---
name: chaos-explorer
description: Survey the failure surface and produce a prioritized chaos-experiment agenda. Partitions the system into failure domains, classifies rehearsal depth (REHEARSED / DEFENDED-UNREHEARSED / UNDEFENDED), and identifies failure classes the existing resilience posture is silent on. Proposes experiments, never executes them.
---

# Chaos Explorer
Survey the failure surface and produce a prioritized chaos-experiment agenda. Partitions the system into failure domains, classifies rehearsal depth (REHEARSED / DEFENDED-UNREHEARSED / UNDEFENDED), and identifies failure classes the existing resilience posture is silent on. Proposes experiments, never executes them.

## Arguments

**Usage:** `/agents:chaos-explorer <directory>`

**Examples:**
- `/agents:chaos-explorer .`
- `/agents:chaos-explorer ./src`
- `/agents:chaos-explorer ./services`
...
```bash
echo "Running chaos exploration on $ARGUMENTS..."
echo "=========================================="
```
...
Run the Chaos Explorer agent on the validated target directory:

**Agent:** chaos-explorer-agent.md
**Model:** Opus
**Target:** $ARGUMENTS
**Prompt tag:** Begin the agent prompt with `[agent:chaos-explorer]` — the agent-metrics hook resolves agent names only from this tag; omitting it captures the agent's metrics nameless.
...
Chaos exploration complete. Review experiment agenda above.
...
Chaos exploration encountered issues.
...
| `definition_name` | `chaos-explorer` | From CDL interface |
...
**CDL Source:** `/Users/aself/uluops/uluops-agent-workflows/udl/cdl/v1/chaos-explorer.command.yaml`
**Agent:** `agents/chaos-explorer-agent.md`

---
*Generated from CDL v2.0.0 | Command: chaos-explorer v1.0.0*
